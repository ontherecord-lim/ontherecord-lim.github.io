(() => {
  ...
  async function loadMarkets() {
    ...
    try {
      const response = await fetch('https://api.frankfurter.app/latest?from=USD&to=KRW,JPY', { cache: 'no-store' });
      if (!response.ok) throw new Error(`FX HTTP ${response.status}`);
      const data = await response.json();
      const krw = Number(data?.rates?.KRW);
      const jpy = Number(data?.rates?.JPY);
      if (!Number.isFinite(krw) || !Number.isFinite(jpy)) throw new Error('Invalid FX data');

      stage.innerHTML = `
        <div class="market-quotes">
          <div class="market-quote"><span class="market-symbol">USD / KRW</span><strong class="market-price">${krw.toLocaleString(isKo ? 'ko-KR' : 'en-US', { maximumFractionDigits: 2 })}</strong></div>
          <div class="market-quote"><span class="market-symbol">USD / JPY</span><strong class="market-price">${jpy.toLocaleString(isKo ? 'ko-KR' : 'en-US', { maximumFractionDigits: 2 })}</strong></div>
        </div>`;
    } catch (error) {
