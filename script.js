(() => {
  ...
  async function loadArticles() {
    try {
      ...
      const tickerItems = articles.map((article) => `<span>${esc(cleanTitle(article.title))}</span><i>◆</i>`).join('');
  ...
  async function loadMarkets() {
    ...
    try {
      ...
      const rates = fxPairs.map((pair) => {
        const history = (grouped.get(pair.quote) || []).sort((a, b) => String(a.date).localeCompare(String(b.date)));
      ...
      stage.innerHTML = rates.map((rate, index) => `
        <div class="market-quote${index === 0 ? ' is-active' : ''}">
          ...
          <span class="market-price">${new Intl.NumberFormat('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(rate.value)}</span>
  ...
  async function loadWeatherCity(selector, latitude, longitude, timezone) {
    ...
    const data = await response.json();
    const root = $(selector);
    root.querySelector('.weather-value').textContent = `${Math.round(Number(data.current.temperature_2m))}°C`;
    root.querySelector('.weather-condition').textContent = weatherLabel(Number(data.current.weather_code));
  }
  ...
  async function loadWeather() {
    try {
      ...
      $$('.weather-condition').forEach((element) => { element.textContent = 'Unavailable'; });
    }
  }
  ...
  async function loadOffTheRecord() {
    ...
    if (!grid) return;
    try {
      const response = await fetch(`data/off-the-record.json?v=${Date.now()}`, { cache: 'no-store' });
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      const data = await response.json();
      const posts = Array.isArray(data.posts) ? data.posts.filter((post) => post?.title) : [];
      if (!posts.length) return;

      grid.innerHTML = posts.slice(0, 6).map((post) => {
        const gatedContentId = post.gatedContentId || 'off-the-record-full-access';
        const hostedContentId = post.hostedContentId || '';
        const image = post.image
          ? `<div class="off-record-cover image-shell"><img src="${esc(post.image)}" alt="${esc(post.imageAlt || post.title)}" loading="lazy"></div>`
          : '';
        const secureContent = hostedContentId
          ? `<div class="off-record-member-content" data-ms-content="${esc(gatedContentId)}" data-ms-bind:style="display:block" style="display:none"><div data-ms-secure-html="${esc(hostedContentId)}"></div></div>`
          : '';
        ...
        return `
          <article class="off-record-card off-record-feature">
            ...
            <div class="off-record-copy">
              <p class="story-index">${esc(post.category || 'Essay')} · ${esc(post.date || '')}</p>
              <h3>${esc(post.title)}</h3>
              ${post.excerpt ? `<p class="off-record-preview">${esc(cleanText(post.excerpt, 240))}</p>` : ''}

              <div class="off-record-paywall" data-ms-content="!${esc(gatedContentId)}">
                <p class="paywall-label">🔒 Subscribers only</p>
                <p>The rest of this Off the Record entry is available to paid members.</p>
                <div class="paywall-actions">
                  <a href="#" class="paywall-primary" data-ms-price:update="prc_off-the-record-full-access-t7da0shc" data-ms-modal="signup">Subscribe to continue reading</a>
                  <a href="#" class="paywall-secondary" data-ms-modal="login">Already a member? Log in</a>
                </div>
              </div>
  ...
  function initNewsletter() {
    ...
    if (close && widget) {
      close.addEventListener('click', () => widget.classList.add('is-hidden'));
    }
    ...
    form.addEventListener('submit', (event) => {
      ...
      message.textContent = 'Newsletter signup is not connected yet. Your email has not been stored.';
      input.value = '';
    });
  }
  ...
  function initCursor() {
    ...
    document.addEventListener('pointermove', (event) => {
      ...
      document.body.classList.toggle('cursor-on-link', Boolean(event.target.closest('a,button,input')));
      const shell = event.target.closest('.image-shell');
      if (shell) {
        const img = $('img', shell);
        if (img) {
          const rect = shell.getBoundingClientRect();
          const ox = ((event.clientX - rect.left) / rect.width - .5) * 6;
          const oy = ((event.clientY - rect.top) / rect.height - .5) * 6;
          img.style.transform = `scale(1.025) translate3d(${ox}px,${oy}px,0)`;
        }
      }
    }, { passive: true });
