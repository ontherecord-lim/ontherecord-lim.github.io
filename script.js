(() => {
    ...
    try {
      ...
      grid.innerHTML = posts.slice(0, 6).map((post) => {
        ...
        const titleHtml = post.url
          ? `<a href="${esc(post.url)}" target="${post.url.startsWith('http') ? '_blank' : '_self'}" rel="${post.url.startsWith('http') ? 'noopener noreferrer' : ''}">${title}</a>`
          : title;
        const readMore = post.url
          ? `<a class="off-record-read" href="${esc(post.url)}" target="${post.url.startsWith('http') ? '_blank' : '_self'}" rel="${post.url.startsWith('http') ? 'noopener noreferrer' : ''}">Read more ↗</a>`
          : '';
        return `
          <article class="off-record-card">
            <p class="story-index">${esc(post.category || 'Essay')} · ${esc(post.date || '')}</p>
            <h3>${titleHtml}</h3>
            ${post.excerpt ? `<p>${esc(cleanText(post.excerpt, 220))}</p>` : ''}
            ${readMore}
          </article>`;
      }).join('');
    } catch (error) {
  ...
  function initNewsletter() {
    ...
    form.addEventListener('submit', (event) => {
      ...
      message.textContent = 'Newsletter sign-up is not connected yet. Your email has not been stored.';
      input.value = '';
    });
  }
