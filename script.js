(() => {
  ...
  async function loadArticles() {
    try {
      ...
      $('#latestGrid').innerHTML = articles.slice(1, 6).map((article) => `
        <article class="latest-item">
          <p class="rubric">${esc(article.category || 'The Korea Herald · Business')}</p>
          <h3 class="latest-item-title"><a href="${esc(article.url)}" target="_blank" rel="noopener noreferrer">${esc(cleanTitle(article.title))}</a></h3>
          <p class="latest-item-deck">${esc(cleanText(article.description, 125))}</p>
      ...
      $('#latestGrid').innerHTML = '<p class="latest-item-deck">Selected reporting is temporarily unavailable.</p>';
    }
  }
